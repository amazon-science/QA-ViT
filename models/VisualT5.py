...
class VisualT5(T5ForConditionalGeneration):
    ...
    def initialize_vision_modules(self, config):
        ...
        self.vision = CLIPVisionTower(config=config, instruction_dim=self.instruction_dim)
        self.vit_type = config['vit_type']
        # Vision-> Language projection module
        self.translation_mlp = None
        if config['hidden_translation_layers'] > 0:
            self.translation_mlp = MLP(self.vision.hidden_size, self.model_dim, self.model_dim,
    ...
    def forward(
        ...
        if encoder_outputs is None:
            input_embeds = self.encoder.embed_tokens(input_ids.to(self.encoder.embed_tokens.weight.device))
            instruct_features = None
            instruct_masks = attention_mask
            # Embed Instruction, if needed
            if self.instruction_embedder is not None:
                outs = self.get_tokenizer()(instructions_list, return_tensors="pt",
                                            padding='longest', truncation=True, max_length=30).to(
                    attention_mask.device)
                instruction_tokens = outs.input_ids
                instruct_masks = outs.attention_mask
                instruct_embeds = self.instruction_embedder(instruction_tokens.
                                                            to(self.encoder.embed_tokens.weight.device))
                instruct_embeds = self.encoder(inputs_embeds=instruct_embeds,
                                               attention_mask=instruct_masks).last_hidden_state
                instruct_features = instruct_embeds
            # Encode vision
            # enable grad if ViT is unfrozen or when we apply qavit
            with torch.set_grad_enabled((not self.freeze_clip) or self.vit_type == 'qavit'):
                image_features = self.vision(instruct_states=instruct_features, instruct_masks=instruct_masks, **images)
                image_atts = torch.ones(image_features.size()[:-1], dtype=torch.long, device=image_features.device)
            ...
            # Encode in LLM
            encoder_outputs = self.encoder(inputs_embeds=input_embeds,
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        if self.vision is not None:
            ...
            # Embed Instruction, if needed
            if self.instruction_embedder is not None:
                outs = self.get_tokenizer()(model_kwargs['instructions_list'], return_tensors="pt",
                                            padding='longest', truncation=True, max_length=30).to(
                    encoder_kwargs['attention_mask'].device)
                instruction_tokens = outs.input_ids
                instruct_masks = outs.attention_mask
                instruct_embeds = self.instruction_embedder(instruction_tokens.to(
                    self.encoder.embed_tokens.weight.device))
                instruct_embeds = self.encoder(inputs_embeds=instruct_embeds,
                                               attention_mask=instruct_masks).last_hidden_state
                instruct_features = instruct_embeds
            image_features = self.vision(instruct_states=instruct_features, instruct_masks=instruct_masks,
                                         **model_kwargs['images'])
            ...
            # Concat image + text
            input_embeds = torch.cat([image_features, input_embeds], dim=1)
            encoder_kwargs['attention_mask'] = torch.cat([image_atts, encoder_kwargs['attention_mask']], dim=1)
        #
