...
class TextVQAEvaluator:
    def __init__(self, dataset_json_file, q_id_list=None):
        self.answer_processor = EvalAIAnswerProcessor()
        self.q_id_list = q_id_list
    ...
    def evaluate_pred_file(self, pred_json_file, return_tuple=False):
        """
        ...
        for entry in pred_json:
            question_id = entry['question_id']
            if (self.q_id_list is not None) and (question_id not in self.q_id_list):
                continue
            pred_answer = self.answer_processor(entry['answer'])
            if is_time(pred_answer):
                pred_answer_fix = fix(pred_answer, ':')
                cnt += 1
                pred_answer = pred_answer_fix
        ...
        if return_tuple:
            return correct_ids, accuracy
        return accuracy


def is_time(ans):
    ans_split = ans.split(':')
    return len(ans_split) > 1
...
def fix(ans, tok=':'):
    ans_split = ans.split(tok)
    ans_split = [s.replace(" ", "") for s in ans_split]
    # add :
    ans = [ans_split[0]]
    for a in range(1, len(ans_split)):
        ans.append(tok + ans_split[a])
    return ''.join(ans)
